	   ...
	   //	   const TString OutPutRootFileName = "ch1_splitted_TF1_WV")
{
  ...
  TH1D* histsWjj[6];
  histsWjj[0] = new TH1D("dataWjj","dataWjj",NBINS, massLEdges);       histsWjj[0]->Sumw2();
  histsWjj[1] = new TH1D("EWKWjj","EWKWjj",NBINS, massLEdges);         histsWjj[1]->Sumw2();
  histsWjj[2] = new TH1D("DibosonWjj","DibosonWjj",NBINS, massLEdges); histsWjj[2]->Sumw2();
  histsWjj[3] = new TH1D("topWjj","topWjj",NBINS, massLEdges);         histsWjj[3]->Sumw2();
  histsWjj[4] = new TH1D("VjetsWjj","VjetsWjj",NBINS, massLEdges);     histsWjj[4]->Sumw2();
  histsWjj[5] = new TH1D("QCDWjj","QCDWjj",NBINS, massLEdges);         histsWjj[5]->Sumw2();

  TH1D* histsZjj[6];
  histsZjj[0] = new TH1D("dataZjj","dataZjj",NBINS, massLEdges);       histsZjj[0]->Sumw2();
  histsZjj[1] = new TH1D("EWKZjj","EWKZjj",NBINS, massLEdges);         histsZjj[1]->Sumw2();
  histsZjj[2] = new TH1D("DibosonZjj","DibosonZjj",NBINS, massLEdges); histsZjj[2]->Sumw2();
  histsZjj[3] = new TH1D("topZjj","topZjj",NBINS, massLEdges);         histsZjj[3]->Sumw2();
  histsZjj[4] = new TH1D("VjetsZjj","VjetsZjj",NBINS, massLEdges);     histsZjj[4]->Sumw2();
  histsZjj[5] = new TH1D("QCDZjj","QCDZjj",NBINS, massLEdges);         histsZjj[5]->Sumw2();

  TH1D* histsWV[6];
  histsWV[0] = new TH1D("dataWV","dataWV",NBINS, massLEdges);       histsWV[0]->Sumw2();
  histsWV[1] = new TH1D("EWKWV","EWKWV",NBINS, massLEdges);         histsWV[1]->Sumw2();
  histsWV[2] = new TH1D("DibosonWV","DibosonWV",NBINS, massLEdges); histsWV[2]->Sumw2();
  histsWV[3] = new TH1D("topWV","topWV",NBINS, massLEdges);         histsWV[3]->Sumw2();
  histsWV[4] = new TH1D("VjetsWV","VjetsWV",NBINS, massLEdges);     histsWV[4]->Sumw2();
  histsWV[5] = new TH1D("QCDWV","QCDWV",NBINS, massLEdges);         histsWV[5]->Sumw2();

  TH1D* histsZV[6];
  histsZV[0] = new TH1D("dataZV","dataZV",NBINS, massLEdges);       histsZV[0]->Sumw2();
  histsZV[1] = new TH1D("EWKZV","EWKZV",NBINS, massLEdges);         histsZV[1]->Sumw2();
  histsZV[2] = new TH1D("DibosonZV","DibosonZV",NBINS, massLEdges); histsZV[2]->Sumw2();
  histsZV[3] = new TH1D("topZV","topZV",NBINS, massLEdges);         histsZV[3]->Sumw2();
  histsZV[4] = new TH1D("VjetsZV","VjetsZV",NBINS, massLEdges);     histsZV[4]->Sumw2();
  histsZV[5] = new TH1D("QCDZV","QCDZV",NBINS, massLEdges);         histsZV[5]->Sumw2();
  ...
  for (size_t isamp=0; isamp<samples.size(); isamp++) {
    ...
    mytree->SetBranchStatus("isResolved",1);
    mytree->SetBranchAddress("isResolved",&isResolved);
    ...
    for(int i = 0; i<mytree->GetEntries(); i++)
    {
      ...
      if ( !( nBTagJet_loose==0 && 
	      ...
	      //vbf_maxpt_jj_m>800) ) continue;
	      //(BosonCentrality_type0>1.0) &&
	      //(vbf_maxpt_jj_m>800) ) ) continue;
	      //(vbf_maxpt_jj_m>500) &&	      
	      //((abs(ZeppenfeldWL_type0)/abs(vbf_maxpt_j2_eta-vbf_maxpt_j1_eta))<0.3) &&
	      //((abs(ZeppenfeldWH)/abs(vbf_maxpt_j2_eta-vbf_maxpt_j1_eta))<0.3) ) ) continue;

      if ( !isResolved) {
	//std::cout << "boosted" << std::endl;

	//if (l_pt1>50 && l_pt2>30) cout << l_eta1 << ", " << l_eta2 << ", " << dilep_m << ", " << mass_llj_PuppiAK8 << ", " << ungroomed_PuppiAK8_jet_pt << ", " << ungroomed_PuppiAK8_jet_eta << ", " << PuppiAK8_jet_tau2tau1 << endl;
	...
	if ( (l_pt1>50 && (((type==0)&&(abs(l_eta1)<2.4)) || ((type==1)&&((abs(l_eta1)<2.5)&&!(abs(l_eta1)>1.4442 && abs(l_eta1)<1.566))))) &&
	     ...
	     //(mass_llj_PuppiAK8>150) ) {
	  
	  //std::cout << mass_llj_PuppiAK8 << std::endl;

	  //ZV
	  if(s->name().EqualTo("data"))	         histsZV[0]->Fill(mass_llj_PuppiAK8,(xsec*otherscale*genWeight*trig_eff_Weight*id_eff_Weight*pu_Weight*btag0Wgt)/(1.0*(nmc-2*nneg)));
	  else if(s->name().EqualTo("WV_EWK"))	 histsZV[1]->Fill(mass_llj_PuppiAK8,(xsec*otherscale*genWeight*trig_eff_Weight*id_eff_Weight*pu_Weight*btag0Wgt)/(1.0*(nmc-2*nneg)));
	  else if(s->name().EqualTo("Diboson"))  histsZV[2]->Fill(mass_llj_PuppiAK8,(xsec*otherscale*genWeight*trig_eff_Weight*id_eff_Weight*pu_Weight*btag0Wgt)/(1.0*(nmc-2*nneg)));
	  else if(s->name().EqualTo("top"))  	 histsZV[3]->Fill(mass_llj_PuppiAK8,(xsec*otherscale*genWeight*trig_eff_Weight*id_eff_Weight*pu_Weight*btag0Wgt)/(1.0*(nmc-2*nneg)));
	  else if(s->name().EqualTo("Vjets"))	 histsZV[4]->Fill(mass_llj_PuppiAK8,(xsec*otherscale*genWeight*trig_eff_Weight*id_eff_Weight*pu_Weight*btag0Wgt)/(1.0*(nmc-2*nneg)));
	  else if(s->name().EqualTo("QCD"))	 histsZV[5]->Fill(mass_llj_PuppiAK8,(xsec*otherscale*genWeight*trig_eff_Weight*id_eff_Weight*pu_Weight*btag0Wgt)/(1.0*(nmc-2*nneg)));

	}
	else if ( (l_pt2<0 && l_pt1>50) && (((type==0)&&(abs(l_eta1)<2.4))||((type==1)&&((abs(l_eta1)<2.5)&&!(abs(l_eta1)>1.4442 && abs(l_eta1)<1.566)))) &&
		  (((type==0)&&(pfMET_Corr>50)) || ((type==1)&&(pfMET_Corr>80))) &&
		  ((ungroomed_PuppiAK8_jet_pt>200)&&(abs(ungroomed_PuppiAK8_jet_eta)<2.4)&&(PuppiAK8_jet_tau2tau1<0.55)) &&
		  (mass_lvj_type0_PuppiAK8>150)) {

	  //WV
	  if(s->name().EqualTo("data"))	         histsWV[0]->Fill(mass_lvj_type0_PuppiAK8,(xsec*otherscale*genWeight*trig_eff_Weight*id_eff_Weight*pu_Weight*btag0Wgt)/(1.0*(nmc-2*nneg)));
	  else if(s->name().EqualTo("WV_EWK"))	 histsWV[1]->Fill(mass_lvj_type0_PuppiAK8,(xsec*otherscale*genWeight*trig_eff_Weight*id_eff_Weight*pu_Weight*btag0Wgt)/(1.0*(nmc-2*nneg)));
	  else if(s->name().EqualTo("Diboson"))  histsWV[2]->Fill(mass_lvj_type0_PuppiAK8,(xsec*otherscale*genWeight*trig_eff_Weight*id_eff_Weight*pu_Weight*btag0Wgt)/(1.0*(nmc-2*nneg)));
	  else if(s->name().EqualTo("top"))  	 histsWV[3]->Fill(mass_lvj_type0_PuppiAK8,(xsec*otherscale*genWeight*trig_eff_Weight*id_eff_Weight*pu_Weight*btag0Wgt)/(1.0*(nmc-2*nneg)));
	  else if(s->name().EqualTo("Vjets"))	 histsWV[4]->Fill(mass_lvj_type0_PuppiAK8,(xsec*otherscale*genWeight*trig_eff_Weight*id_eff_Weight*pu_Weight*btag0Wgt)/(1.0*(nmc-2*nneg)));
	  else if(s->name().EqualTo("QCD"))	 histsWV[5]->Fill(mass_lvj_type0_PuppiAK8,(xsec*otherscale*genWeight*trig_eff_Weight*id_eff_Weight*pu_Weight*btag0Wgt)/(1.0*(nmc-2*nneg)));

	}
	//else {
	//cout << l_pt1 << ", " << l_pt2 << endl;
	//}
	...
	if ( (l_pt1>50 && (((type==0)&&(abs(l_eta1)<2.4)) || ((type==1)&&((abs(l_eta1)<2.5)&&!(abs(l_eta1)>1.4442 && abs(l_eta1)<1.566))))) &&
             (l_pt2>30 && (((type==0)&&(abs(l_eta2)<2.4)) || ((type==1)&&((abs(l_eta2)<2.5)&&!(abs(l_eta2)>1.4442 && abs(l_eta2)<1.566))))) &&
	     (dilep_m>76 && dilep_m<107) &&
	     (mass_llj_PuppiAK8>150) ){ // that's where I put the dijet

	  //Zjj	  
	  if(s->name().EqualTo("data"))	         histsZjj[0]->Fill(mass_llj_PuppiAK8,(xsec*otherscale*genWeight*trig_eff_Weight*id_eff_Weight*pu_Weight*btag0Wgt)/(1.0*(nmc-2*nneg)));
	  else if(s->name().EqualTo("WV_EWK"))	 histsZjj[1]->Fill(mass_llj_PuppiAK8,(xsec*otherscale*genWeight*trig_eff_Weight*id_eff_Weight*pu_Weight*btag0Wgt)/(1.0*(nmc-2*nneg)));
	  else if(s->name().EqualTo("Diboson"))  histsZjj[2]->Fill(mass_llj_PuppiAK8,(xsec*otherscale*genWeight*trig_eff_Weight*id_eff_Weight*pu_Weight*btag0Wgt)/(1.0*(nmc-2*nneg)));
	  else if(s->name().EqualTo("top"))  	 histsZjj[3]->Fill(mass_llj_PuppiAK8,(xsec*otherscale*genWeight*trig_eff_Weight*id_eff_Weight*pu_Weight*btag0Wgt)/(1.0*(nmc-2*nneg)));
	  else if(s->name().EqualTo("Vjets"))	 histsZjj[4]->Fill(mass_llj_PuppiAK8,(xsec*otherscale*genWeight*trig_eff_Weight*id_eff_Weight*pu_Weight*btag0Wgt)/(1.0*(nmc-2*nneg)));
	  else if(s->name().EqualTo("QCD"))	 histsZjj[5]->Fill(mass_llj_PuppiAK8,(xsec*otherscale*genWeight*trig_eff_Weight*id_eff_Weight*pu_Weight*btag0Wgt)/(1.0*(nmc-2*nneg)));
	  ...
	  //Wjj
	  if(s->name().EqualTo("data"))	         histsWjj[0]->Fill(mass_lvj_type0_PuppiAK8,(xsec*otherscale*genWeight*trig_eff_Weight*id_eff_Weight*pu_Weight*btag0Wgt)/(1.0*(nmc-2*nneg)));
	  else if(s->name().EqualTo("WV_EWK"))	 histsWjj[1]->Fill(mass_lvj_type0_PuppiAK8,(xsec*otherscale*genWeight*trig_eff_Weight*id_eff_Weight*pu_Weight*btag0Wgt)/(1.0*(nmc-2*nneg)));
	  else if(s->name().EqualTo("Diboson"))  histsWjj[2]->Fill(mass_lvj_type0_PuppiAK8,(xsec*otherscale*genWeight*trig_eff_Weight*id_eff_Weight*pu_Weight*btag0Wgt)/(1.0*(nmc-2*nneg)));
	  else if(s->name().EqualTo("top"))  	 histsWjj[3]->Fill(mass_lvj_type0_PuppiAK8,(xsec*otherscale*genWeight*trig_eff_Weight*id_eff_Weight*pu_Weight*btag0Wgt)/(1.0*(nmc-2*nneg)));
	  else if(s->name().EqualTo("Vjets"))	 histsWjj[4]->Fill(mass_lvj_type0_PuppiAK8,(xsec*otherscale*genWeight*trig_eff_Weight*id_eff_Weight*pu_Weight*btag0Wgt)/(1.0*(nmc-2*nneg)));
	  else if(s->name().EqualTo("QCD"))	 histsWjj[5]->Fill(mass_lvj_type0_PuppiAK8,(xsec*otherscale*genWeight*trig_eff_Weight*id_eff_Weight*pu_Weight*btag0Wgt)/(1.0*(nmc-2*nneg)));
  ...
  for (int i=0; i<6; i++)
    {
      histsWjj[i]->SetBinContent(NBINS,histsWjj[i]->GetBinContent(NBINS)+histsWjj[i]->GetBinContent(NBINS+1));
      histsZjj[i]->SetBinContent(NBINS,histsZjj[i]->GetBinContent(NBINS)+histsZjj[i]->GetBinContent(NBINS+1));
      histsWV[i]->SetBinContent(NBINS,histsWV[i]->GetBinContent(NBINS)+histsWV[i]->GetBinContent(NBINS+1));
      histsZV[i]->SetBinContent(NBINS,histsZV[i]->GetBinContent(NBINS)+histsZV[i]->GetBinContent(NBINS+1));
      //hists[i]->Write();
    }
  
  TH1 *h = 0;//mit->second;
  std::cout << "Did we ever get here? " << std::endl;
  TFile *outFile = new TFile("SMEWK_test.root","RECREATE"); 
  
  TString OutRootFileSuffix = "_NoBinbyBin";
  TString OutPutRootFileName = "SMEWK_test";
  ...
  f.cd("Wjj");
  for (int i=0; i<6; i++) { histsWjj[i]->Write(); }
  f.cd("Zjj");
  for (int i=0; i<6; i++) { histsZjj[i]->Write(); }
  f.cd("WV");
  for (int i=0; i<6; i++) { histsWV[i]->Write(); }
  f.cd("ZV");
  for (int i=0; i<6; i++) { histsZV[i]->Write(); }
  ...
  sprintf(outputLimitsShape,"SMEWK_test_Wjj.txt");
  ...
  newcardShape << Form("shapes * * SMEWK_test.root Wjj/$PROCESS Wjj/$PROCESS_$SYSTEMATIC\n");
  newcardShape << Form("shapes data_obs * SMEWK_test.root Wjj/dataWjj\n");

  newcardShape << Form("bin bin1\n");
  newcardShape << Form("Observation %d\n", -1/*(int)histo_Data->GetBinContent(nb)*/);
  newcardShape << Form("bin bin1 bin1 bin1 bin1 bin1\n");
  newcardShape << Form("process EWKWjj DibosonWjj topWjj VjetsWjj QCDWjj\n");
  newcardShape << Form("process 0 1 2 3 4 \n");
  newcardShape << Form("rate %8.5f %8.5f  %8.5f  %8.5f  %8.5f\n",-1.,-1.,-1.,-1.,-1.) ;
  
  //newcardShape << Form("lumi lnN 1.02 1.02 1.02 1.02 1.02\n");

  newcardShape.close();

  //Zjj
  sprintf(outputLimitsShape,"SMEWK_test_Zjj.txt");
  ...
  newcardShape << Form("shapes * * SMEWK_test.root Zjj/$PROCESS Zjj/$PROCESS_$SYSTEMATIC\n");
  newcardShape << Form("shapes data_obs * SMEWK_test.root Zjj/dataZjj\n");

  newcardShape << Form("bin bin1\n");
  newcardShape << Form("Observation %d\n", -1/*(int)histo_Data->GetBinContent(nb)*/);
  newcardShape << Form("bin bin1 bin1 bin1 bin1 bin1\n");
  newcardShape << Form("process EWKZjj DibosonZjj topZjj VjetsZjj QCDZjj\n");
  newcardShape << Form("process 0 1 2 3 4 \n");
  newcardShape << Form("rate %8.5f %8.5f  %8.5f  %8.5f  %8.5f\n",-1.,-1.,-1.,-1.,-1.) ;
  ...
  //WV
  sprintf(outputLimitsShape,"SMEWK_test_WV.txt");
  ...
  newcardShape << Form("shapes * * SMEWK_test.root WV/$PROCESS WV/$PROCESS_$SYSTEMATIC\n");
  newcardShape << Form("shapes data_obs * SMEWK_test.root WV/dataWV\n");

  newcardShape << Form("bin bin1\n");
  newcardShape << Form("Observation %d\n", -1/*(int)histo_Data->GetBinContent(nb)*/);
  newcardShape << Form("bin bin1 bin1 bin1 bin1 bin1\n");
  newcardShape << Form("process EWKWV DibosonWV topWV VjetsWV QCDWV\n");
  newcardShape << Form("process 0 1 2 3 4 \n");
  newcardShape << Form("rate %8.5f %8.5f  %8.5f  %8.5f  %8.5f\n",-1.,-1.,-1.,-1.,-1.) ;
  ...
  sprintf(outputLimitsShape,"SMEWK_test_ZV.txt");
  ...
  newcardShape << Form("shapes * * SMEWK_test.root ZV/$PROCESS ZV/$PROCESS_$SYSTEMATIC\n");
  newcardShape << Form("shapes data_obs * SMEWK_test.root ZV/dataZV\n");

  newcardShape << Form("bin bin1\n");
  newcardShape << Form("Observation %d\n", -1/*(int)histo_Data->GetBinContent(nb)*/);
  newcardShape << Form("bin bin1 bin1 bin1 bin1 bin1\n");
  newcardShape << Form("process EWKZV DibosonZV topZV VjetsZV QCDZV\n");
  newcardShape << Form("process 0 1 2 3 4 \n");
  newcardShape << Form("rate %8.5f %8.5f  %8.5f  %8.5f  %8.5f\n",-1.,-1.,-1.,-1.,-1.) ;
...
void Resolved_GetCard_test()
{
  int start_s=clock(); 
  model("DibosonResolvedSamples13TeV.txt");
